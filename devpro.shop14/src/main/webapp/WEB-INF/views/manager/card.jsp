...
<link rel="stylesheet" type="text/css"
	href="${base}/user/fontawesome-free-5.15.3-web/css/all.css">
<!-- bootstrap js -->
...
<script type="text/javascript"
	href="${base}/user/js/jquery.sticky-kit.js"></script>
<script type="text/javascript"
							...
							<c:forEach var="product" items="${cart.getCartItems() }">
								<tr>
									...
									<td><a href="#"><img class="mr-3 img-thumbnail"
											src="${product.getAvatar() }"></a></td>
									<td><a href="${base }/product/${product.getProductId()}">${product.getProductName() }</a></td>
									<td><label style="text-align: justify;">
											${product.getShortDesc() } </label></td>
									<td><label> ${product.getQuanlity() } </label></td>
									<td>
										<p>${product.getPriceUnit() }</p>
									</td>
								</tr>
							</c:forEach>
							<tr>
								<th colspan="5"><label> Tổng giá trị đơn hàng: </label></th>
								<td><p>${cart.getTotalPrice() }</p></td>
							</tr>
						</tbody>
					</table>
				</div>
					...
					<c:choose>
						<c:when test="${cart != null }">
							<form action="${base }/view/access" method="post">
								<div class="form-group">
									<label for="exampleFormControlInput1">Full Name</label> <input
										type="text" name="customerName" class="form-control"
										id="inputName" placeholder="Nguyen Duc Son">
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label for="exampleFormControlSelect1">Select Address</label> <select
										class="form-control" id="inputAdd" name="customerAddr">
								...
								<div class="row justify-content-center">
									<button onclick="alertMess()" type="submit"
										class="buy text-white">XÁC NHẬN ĐƠN HÀNG</button>
								</div>
							</form>
						</c:when>
						...
						<c:otherwise>
							<form action="${base }/view/access" method="post">
								<div class="form-group">
									<label for="exampleFormControlInput1">Full Name</label> <input
										type="text" name="customerName" readonly="readonly"
										class="form-control" id="inputName"
										placeholder="${order.customerName }">
								</div>
								<div class="form-group">
									<label for="exampleFormControlInput1">Email address</label> <input
										type="email" name="customerEmail" readonly="readonly"
								...
								<div class="row justify-content-center">
									<button type="submit" class="buy text-white">XÁC NHẬN
										ĐƠN HÀNG</button>
								</div>
							</form>
						</c:otherwise>
